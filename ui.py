...
class QuizInterface:
    ...
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.question_text = self.canvas.create_text(150, 125, width=280, text="Title", font=("Ariel", 20, "italic"))
        self.canvas.config(bg="white", highlightthickness=0)
        self.canvas.grid(row=1, column=0, columnspan=2, pady=50)

        self.score_label = Label(text="Score: 0", fg="white", bg=THEME_COLOR)
        # self.score_label.config(padx=20, pady=20, highlightthickness=0)
        self.score_label.grid(row=0, column=1)

        cross_img = PhotoImage(file="images/false.png")
        self.unknown_button = Button(image=cross_img, highlightthickness=0, command=self.false_pressed)
